			...
			for profile in Path(ff_path).glob("*.default"):
				...
				if places.exists():
					try:
						...
						shutil.copy2(str(places), str(temp_db))
						conn = sqlite3.connect(str(temp_db))
						cursor = conn.cursor()
...
class ForenXApp:
	def __init__(self, root):
		self.root = root
		self.root.title("🔍 ForenX AI - Complete Digital Forensics Suite")
		...
		self.pages = []
		self.evidence_data = {}
		self.output_dir = "forensic_output"
	...
	def setup_ui(self):
		...
		# Top navigation bar
		self.nav_frame = Frame(self.main_frame, bg='#2d2d2d', height=50)
		self.nav_frame.pack(fill=X, side=TOP)
		self.nav_frame.pack_propagate(False)
        
		# Back button
		self.back_btn = Button(self.nav_frame, text="◀ BACK", command=self.go_back,
							  bg='#3d3d3d', fg='#00ff00', font=('Arial', 10, 'bold'),
							  cursor="hand2")
		self.back_btn.pack(side=LEFT, padx=10, pady=10)
        
		# Next button
		self.next_btn = Button(self.nav_frame, text="NEXT ▶", command=self.go_next,
							  bg='#3d3d3d', fg='#00ff00', font=('Arial', 10, 'bold'),
							  cursor="hand2")
		self.next_btn.pack(side=RIGHT, padx=10, pady=10)
        
		# Page indicator
		self.page_label = Label(self.nav_frame, text="Page 1/?", 
							   bg='#2d2d2d', fg='#00ff00', font=('Arial', 10))
		self.page_label.pack(side=RIGHT, padx=10)
        
		# Content area
		self.content_frame = Frame(self.main_frame, bg='#1a1a1a')
		self.content_frame.pack(fill=BOTH, expand=True, padx=10, pady=10)
        
		# Status bar
		self.status_frame = Frame(self.main_frame, bg='#2d2d2d', height=30)
	...
	# (rest of the class omitted for brevity; full app code preserved in file)
